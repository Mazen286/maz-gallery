import Link from "next/link"
import { MoodPlayer } from "@/app/cafe-maz/cafe/MoodPlayer"

export function CafeMaz() {
  return (
    <>
      <p>
        My friend Gino sent me a video of Robert Rodriguez. The director was
        talking about a menu he keeps at his house. He hands it to guests when
        they come over and watches them try to figure out if he&apos;s joking.
        He&apos;s not. He just thinks hospitality should feel like an event.
      </p>

      <p>
        I&apos;ve always jokingly offered coffee from &ldquo;Café Maz&rdquo;, me
        walking to the kitchen to make whatever you&apos;re feeling. After
        Gino&apos;s video, the joke felt less like a joke. So I made the menu.
      </p>

      <p>This is the story of why.</p>

      <hr />

      <h2>Diyafa</h2>

      <p>
        Some of my best memories are nights in Amman. Guests around the table,
        coffee on the stove, cardamom in the air. Coffee, then tea, then
        coffee, then tea, into the night. Plates of fruit and small snacks
        between the rounds. Never asked for, always there. Always more than
        enough. Whether it was friends or family, every night had that rhythm.
        That&apos;s what cemented it for me: hospitality lives in the cup that
        keeps refilling.
      </p>

      <p>
        &ldquo;Tafaddal&rdquo; was the word for everything in my house.{" "}
        <em>Please. Go ahead. Be my guest. Take this.</em> There&apos;s no
        clean English equivalent. It&apos;s what you said handing someone tea,
        opening a door, offering the last piece of bread. &ldquo;Would you
        like...&rdquo; was rude. You don&apos;t ask the guest to want. You
        give, they receive. The host&apos;s job is to decide what&apos;s
        needed.
      </p>

      <p>
        That&apos;s what I grew up with. Hospitality wasn&apos;t politeness. It
        was an obligation, wrapped in care. You don&apos;t just open the door
        for someone. You offer them coffee before they sit down. You
        don&apos;t ask what they want. You make what they need.
      </p>

      <p>
        A menu is the opposite of all of that, of course. A menu asks the
        guest to choose. That tension is half the joke of Café Maz. The menu
        is for guests who already know. If you don&apos;t, <em>tafaddal</em>.
        I&apos;ll decide for you.
      </p>

      <p>
        So when I started having friends over in my own apartment, the joke
        about Café Maz wasn&apos;t really a joke. It was me trying to recreate
        the table I grew up at. And when Gino&apos;s video showed up, I
        realized the joke could become the project.
      </p>

      <hr />

      <h2>The Set Table Is a Love Letter</h2>

      <p>
        I could have hand-written a menu on the fridge. I could have
        hand-written nothing at all and just kept asking people what they
        wanted. Instead I built five pages (an index, a printable A5 menu, a
        recipe book, a flavor lab, an AI tool that brainstorms hookah combos
        with me, picking blends for whatever mood I&apos;m trying to set) and
        curated six Spotify playlists to go with them.
      </p>

      <p>
        Because the design was the gift. Hospitality is theater plus sincerity,
        and the production value matters. You don&apos;t text someone a wedding
        invitation. You don&apos;t serve tea in a paper cup. The set table is a
        love letter, and the love letter takes time to fold.
      </p>

      <p>
        I knew nobody would spend more than a few minutes looking at the menu.
        Friends come over, scan the drinks, point at something, talk about
        something else. The hours I poured into the brand system, the
        typography, the print pipeline would be visible for maybe forty
        seconds. That asymmetry is the gift. The more invisible the craft, the
        more it counts.
      </p>

      <p>
        The design itself is a candlelit Levantine lounge: coffee-black
        background, brass accents, Cinzel for the wordmarks, Amiri for the
        Arabic, italic Cormorant for the small things. Bilingual everywhere,
        because that&apos;s how the house sounds. There&apos;s a signature
        drink: pistachio latte with orange blossom water, marked ✦ on the
        menu. It&apos;s the thing I&apos;m proudest of making. Pistachios
        blended into a cream. More crushed by hand with a mortar and pestle,
        scattered on top.
      </p>

      <hr />

      <h2>Music as Architecture</h2>

      <p>
        My dad always had Umm Kulthum playing through the house. Guests would
        come over, we&apos;d play tarneeb, sometimes nothing in particular was
        happening. She was always there, working her way through{" "}
        <em>Al-Atlal</em> in the background of everything. I bought one of her
        records for my own collection recently. The cover is faded. It still
        plays beautifully.
      </p>

      <p>
        The cafes in Amman play Arabic music too, but the curation often
        misses. Tarab at eleven in the morning when you wanted something
        brighter. Modern pop when the room had settled into something slower.
        Music as background instead of as architecture.
      </p>

      <p>
        So I built six playlists: one for each mood Café Maz might hold across
        a day.
      </p>

      <p>Here&apos;s the picker from the cafe.</p>

      <div className="not-prose my-8">
        <MoodPlayer />
      </div>

      <hr />

      <h2>On the Hookah</h2>

      <p>
        About the hookah, briefly. It&apos;s bad for you. I know. The
        criticism is fair and I&apos;m not going to argue it on health
        grounds.
      </p>

      <p>
        But it&apos;s also half of Levantine evening hospitality. The thing
        that turns a two-hour visit into a four-hour visit. The craft is real:
        bowl packing, coal rotation, blend ratios. Cutting hookah out of a
        Palestinian hospitality project to avoid offending people who
        didn&apos;t grow up with it would be a kind of cultural compression
        I&apos;m not willing to do.
      </p>

      <p>
        The quality matters as much as the ritual. The hookah I use, the coals
        I burn, the ingredients in every blend. I don&apos;t compromise on any
        of it. Most lounges don&apos;t carry these brands, and the ones that
        do charge a premium for them. Every blend gets tested before it lands
        on the menu. The point isn&apos;t to have hookah at the cafe. The
        point is for the bowl to be worth sitting with.
      </p>

      <p>
        The six bowls on the menu are named after Palestinian places and
        traditions. Karm al-Khalil for Hebron&apos;s vineyards. Knafeh Nabulsi
        for the syrup-soaked pastry from Nablus. Sahlab Layl for the winter
        milk drink, late at night. Each one is a small love letter.
      </p>

      <hr />

      <h2>The Door</h2>

      <p>
        This is what I&apos;ve learned: the most personal projects make the
        most generous gifts. I built{" "}
        <Link href="/cafe-maz/cafe">Café Maz</Link>{" "}for the people who already
        walk through my door. If you&apos;re one of them, <em>tafaddal</em>.
        You don&apos;t have to ask.
      </p>
    </>
  )
}
